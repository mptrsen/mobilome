...
sub Wanted{
  if(/.*\.out$/ | ($file_ok==1)){
	...
	print OUTSTAT "Sequence\tFamily\tElement\tLength\tFragments\tCopies\tSolo_LTR\tTotal_Bp\tCover\n";
      }
	...
	foreach $elem (sort keys %{$h{$contig}{$family}}){
	  ...
	  foreach $fam (@{$family_list{$bigfam}}){
	    ...
	    print OUTSTAT "$contig\t###",$fam,"\tAll_elements\tNA";
	    for($i=0;$i<=1;$i++){
	  ...
	  print OUTSTAT "$contig\t######",$bigfam,"\tAll_elements\tNA";
	  for($i=0;$i<=1;$i++){
	  ...
	  print OUTSTAT "$percent\n\n";
        ...
        print OUTSTAT "$contig\t#########Type:EVERYTHING_TE\tAll_elements\tNA";
        for($i=0;$i<=1;$i++){
	  ...
	  print OUTSTAT "$contig\t######Type:",$nonTE,"\tAll_elements\tNA\tNA\t$non_TE_count{$contig}{$nonTE}\tNA\t$non_TE_length{$contig}{$nonTE}\t";
